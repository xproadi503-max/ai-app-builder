...
import { saveBuildRecord } from "../../../lib/buildHistory";
...
function sanitizeRepoName(rawName) {
  let base = (rawName || "").replace(/\.(zip|jsx?|tsx?)$/i, "");
  base = base.replace(/[^a-zA-Z0-9._-]+/g, "-").replace(/-+/g, "-").replace(/^[-.]+|[-.]+$/g, "");
  if (!base) base = "ai-build-" + Date.now();
  return base.slice(0, 90);
}

async function createUniqueRepo(ghToken, baseName) {
  let name = baseName;
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    const nameTaken = JSON.stringify(errData).toLowerCase().includes("already exists") || errData.errors?.some((e) => e.field === "name");
    if (!nameTaken) return { ok: false, name, errData };
    name = `${baseName}-${attempt + 2}`; // -2, -3, -4...
  }
  return { ok: false, name, errData: { message: "Naam available nahi mila, bahut retries ho gaye" } };
}
...
export async function POST(request) {
  ...
  try {
    ...
    const repoBaseName = sanitizeRepoName(sourceName);

    const createResult = await createUniqueRepo(ghToken, repoBaseName);
    if (!createResult.ok) {
      return Response.json({ error: "Repo nahi ban paya: " + JSON.stringify(createResult.errData) }, { status: 500 });
    }
    const repoName = createResult.name;
    ...
    await saveBuildRecord({ userLogin: owner, owner, repoName, projectType: type, sourceName });
